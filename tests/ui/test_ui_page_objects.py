...
@pytest.mark.indeindependent_ui1
def test_create_account(fresh_driver):
    create_account = ShopPage(fresh_driver)
    create_account.create_new_account("First", "Last", "email@email.com", "correct_password" )
    create_account.close()
    

@pytest.mark.independent_ui1
def test_search_for_existing_product(fresh_driver):
...
def test_add_to_cart_functionality(fresh_driver):
    existing_product = ShopPage(fresh_driver)
...
def test_search_for_product_empty_query(fresh_driver):
...
@pytest.mark.independent_ui
def test_search_for_nonexisting_product(fresh_driver):
...
@pytest.mark.independent_ui
def test_add_item_via_search(fresh_driver):
